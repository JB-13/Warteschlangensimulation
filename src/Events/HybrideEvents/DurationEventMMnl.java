...
public class DurationEventMMnl extends Event {
    ...
    public DurationEventMMnl(double time, Clock c, Service s, int position, EventQueue q, OutputStatisticsH out, InputStatistics inB) {
        super(time, c);
        service = s;
    ...
    public DurationEventMMnl(double time, int position) {
        super(time);
        this.resourceId = position;
    }
    ...
    public void action() {
        ...
        if (!queue.isWaitingQueueEmpty()) {
            ...
            queue.insertEvent(new DurationEventMMnl(newFinished, clock, service, resourceId, queue, stats, genB));
